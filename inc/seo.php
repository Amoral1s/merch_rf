...
class True_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	// function start_el( &$output, $item, $depth = 0, $args = NULL, $id = 0 ) {
		...
		if (
			$item->current == 1 || 
			strlen($item->url) < 2 || 
			untrailingslashit($_SERVER['REQUEST_URI']) == untrailingslashit(parse_url($item->url, PHP_URL_PATH))
		) {
...
global $post;
$pattern = "<iframe";
$replacement = '<iframe itemscope itemprop="VideoObject" itemtype="https://schema.org/VideoObject"';
$content = str_replace($pattern, $replacement, $content);
return $content;
}
...
add_filter('wpseo_breadcrumb_links', 'remove_duplicate_home_breadcrumb');
function remove_duplicate_home_breadcrumb($links) {
    if (count($links) > 1 && $links[0]['url'] == $links[1]['url']) {
        array_shift($links); // Удалить второй элемент, если это дублированная "Главная"
    }
    return $links;
}